      ...
      <div className="container mx-auto px-6 relative z-10">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 mb-20">
          ...
          <div className="col-span-1 lg:col-span-2">
            ...
            {/* T10 — distinct closing line, not a copy of hero bio */}
            <p className="text-muted max-w-sm font-body font-light leading-relaxed mb-8">
              Building culturally-rooted software from Addis Ababa, Ethiopia.
            </p>

            {/* T9 — clock uses suppressHydrationWarning to avoid "Loading..." flash */}
            <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 text-muted text-xs sm:text-sm font-medium tracking-widest uppercase">
              ...
              <span suppressHydrationWarning>{time ?? "——:——"} EAT</span>
            </div>
          </div>

          {/* T3 — Navigation: Home / About / Skills / Portfolio / Contact */}
          <div>
            ...
            <ul className="space-y-4">
              {navItems.map((item) => (
                <li key={item.label}>
                  <a
                    href={item.href}
                    className="text-muted hover:text-accent-gold transition-colors block w-fit"
                  >
                    {item.label}
                  </a>
                </li>
